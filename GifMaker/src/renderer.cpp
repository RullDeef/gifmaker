...
constexpr auto M_2PI = 6.28318530718958647692;
...
static bool CheckPathIntersection(std::vector<PathPoint> path, FloatPoint targetPoint, int& maxOffset)
{
    int counter = 0;
    ...
    for (int i = 1; i < path.size(); i++)
    {
        if (path[i].type == PathPoint::Type::LINE)
        {
            ...
            if (std::abs(lineStart.y - lineEnd.y) > 0.01f)
            {
                ...
                if (((lineStart.y < targetPoint.y && targetPoint.y <= lineEnd.y) ||
                    ...
                    && (lineStart.x >= targetPoint.x || lineEnd.x >= targetPoint.x))
                {
                    FloatPoint lineDir = lineEnd - lineStart;
                    float intersectionX = lineStart.x + (targetPoint.y - lineStart.y) / lineDir.y * lineDir.x;

                    if (intersectionX > targetPoint.x)
                    {
                        counter++;
                        maxOffset = std::min<int>(maxOffset, intersectionX - targetPoint.x);
                    }
                }
            }
        }
        else if (path[i].type == PathPoint::Type::ARC_CENTER)
        {
            ...
            if (center.y - radius < targetPoint.y && targetPoint.y < center.y + radius)
            {
                ...
                float startAngle = atan2(path[i - 1].y - center.y, path[i - 1].x - center.x);
                if (startAngle < 0.0f)
                    startAngle += M_2PI;
                float endAngle = startAngle + deltaAngle;

                FloatPoint endPoint = center + FloatPoint::Polar(radius, endAngle);
                ...
                if (distToTarget2 >= radius * radius && targetPoint.x < center.x)
                {
                    ...
                    if (hasIntersection)
                    {
                        counter++;
                        // update offset
                        float distToCircle = center.x - targetPoint.x - sqrt(radius * radius - (targetPoint.y - center.y) * (targetPoint.y - center.y));
...
void Renderer::Fill()
{
    for (int row = 0; row < height; row++)
    {
        for (int col = 0; col < width; col++)
        {
            ...
            if (CheckPathIntersection(path, targetPoint, maxOffset))
            {
                // blend pixels
...
void Renderer::Arc(FloatPoint center, float radius, float startAngle, float endAngle, bool clockwise)
{
    ...
    FloatPoint startPoint = center + FloatPoint::Polar(radius, startAngle);
    FloatPoint endPoint = center + FloatPoint::Polar(radius, endAngle);
